...
const Enrollments = () => {
    ...
    const fetchEnrollments = async () => {
        try {
            ...
            toast.error(t('admin.enrollments.error.load_failed'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const getStatusBadge = (status) => {
        ...
        return (
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${isCompleted ? 'bg-emerald-100 text-emerald-800' : 'bg-blue-100 text-blue-800'}`}>
                ...
                {isCompleted ? t('admin.enrollments.filters.completed') : t('admin.enrollments.filters.in_progress')}
            </span>
        );
    };
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">{t('admin.enrollments.title')}</h1>
                            <p className="text-slate-600 dark:text-slate-400 mt-1">{t('admin.enrollments.subtitle')}</p>
                        </div>
                        <button
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder={t('admin.enrollments.search_placeholder')}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                    {t('admin.enrollments.filters.completion_status')}
                                </label>
                                <select
                                    value={filterStatus}
                                ...
                                >
                                    <option value="all">{t('admin.enrollments.filters.all_status')}</option>
                                    <option value="IN_PROGRESS">{t('admin.enrollments.filters.in_progress')}</option>
                                    <option value="COMPLETED">{t('admin.enrollments.filters.completed')}</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
                    {loading ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="mt-4 text-slate-500 dark:text-slate-400">{t('admin.enrollments.loading')}</p>
                        </div>
                    ) : enrollments.length === 0 ? (
                        <div className="p-12 text-center">
                            <FileText className="w-12 h-12 text-slate-300 dark:text-slate-700 mx-auto mb-4" />
                            <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('admin.enrollments.empty.title')}</h3>
                            <p className="text-slate-500 dark:text-slate-400">{t('admin.enrollments.empty.subtitle')}</p>
                        </div>
                    ) : (
                        <>
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 text-slate-500 dark:text-slate-400">
                                        <tr>
                                            <th className="text-left p-4 text-xs font-bold uppercase tracking-wider">{t('admin.enrollments.table.student')}</th>
                                            <th className="text-left p-4 text-xs font-bold uppercase tracking-wider">{t('admin.enrollments.table.course')}</th>
                                            <th className="text-left p-4 text-xs font-bold uppercase tracking-wider">{t('admin.enrollments.table.progress')}</th>
                                            <th className="text-left p-4 text-xs font-bold uppercase tracking-wider">{t('admin.enrollments.table.status')}</th>
                                            <th className="text-left p-4 text-xs font-bold uppercase tracking-wider">{t('admin.enrollments.table.enrolled_date')}</th>
                                            <th className="text-left p-4 text-xs font-bold uppercase tracking-wider">{t('admin.enrollments.table.actions')}</th>
                                        </tr>
                                    </thead>
                                            ...
                                            >
                                                ...
                                                <td className="p-4">
                                                    <div className="max-w-xs">
                                                        ...
                                                        <div className="text-xs text-slate-500 dark:text-slate-400">
                                                            {t('admin.courses.card.by')} {enrollment.course?.instructor?.firstName} {enrollment.course?.instructor?.lastName}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="p-4">
                                                    <div className="flex flex-col gap-1 w-32">
                                                        <div className="flex items-center justify-between text-[10px] font-black text-slate-400">
                                                            <span>{t('admin.enrollments.table.progress_label')}</span>
                                                ...
                                                <td className="p-4">
                                                    <button
                                                        ...
                                                        title={t('admin.enrollments.actions.revoke')}
                                                    >
                            ...
                            {totalPages > 1 && (
                                <div className="flex items-center justify-between p-6 bg-slate-50/50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800">
                                    <div className="text-sm font-medium text-slate-500 dark:text-slate-400">
                                        {t('admin.enrollments.pagination.showing', { current: currentPage, total: totalPages })}
                                    </div>
